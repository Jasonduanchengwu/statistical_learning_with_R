...
```{r}
seq(1, 30) %>% 
  # use = "pairwise.complete.obs" only uses obs with non-NAs
...
this code is uploaded, so we git pull source repo. **
can try to use map instead of for loop to speed up the process.**
...
This code will be shared with us**
data is a time series, thus the data should be split by time. Write own fold spliting function**
tune the mixture, as long as it is not 0/1 it is still enet.**
