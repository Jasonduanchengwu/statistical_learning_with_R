...
```
**Answer: ** To comment on the most important component, the principle component 1 which captures the maximum variance in the data. In this context all the variables namely in the range of the diseased group have similar and substantial contributions to PC1, indicated by the length of the bars. They are all colored red, meaning their contribution is marked as "FALSE", this could mean their contribution to PC1 is negative which matches with the information that we have known from the data documentation.
...
**Answer: ** It is apparent that the two groups are well separated in the PCA plot. There is clear seggregation between the two groups.
...
**Answer: ** With UMAP, the local neighborhood is preserved and it will not be strictly linear in nature. It is clear that in this case the clusters seems to form oval patterns while still showing the separation of the two distinct groups. There is clear seggregation between the two groups With cluster 2 having a having much more spread.
...
**Answer: ** To identify the genes that differ the most across the two groups, a linear regression analysis on each gene can be used to determine the p-value of the association between the gene and the group. The heatmap shows that the expression of these genes are different across the two groups. We can see that there are clearly more positive values in the healthy group and more negative values in the disease group. This suggests that the genes are differentially expressed across the two groups. And the IDs confirmed that the genes are differentially expressed across the two groups as we know that ID1-20 is disease and ID21-40 is healthy which is reflected in the heatmap as well.