...
enet_mod <- 
  ...
  linear_reg(penalty = tune(), mixture = 0.99) %>% 
  set_engine("glmnet")
enet_mod
...
rf_fit <- tune_grid(rf_wf, resamples = month_folds, grid = rf_grid, metrics = metric_set(rmse, rsq)) 
...
| Baseline | NA | 0.35 |
| AR(5) | 0.699 | 0.688 |
| Random Forest | 0.343 | 0.651 |
| Boosting |  0.387 | 0.682 |

**Answer: ** The best performing model is the AR(5) model with a test R2 of 0.688. The boosting model is the second best with a test R2 of 0.682. The random forest model is the worst performing model with a test R2 of 0.651. The baseline model has a test R2 of 0.35. There are evidently huge improvement between the baseline model and the rest of the more elaborate models.

Noteably, Random Forest and Boosting models have a significantly lower CV R2 than the test R2. This could be a sign of underfitting possbily. Due to my limited computing resources I am not able to fully explore the hyperparameter space for the optimal Random Forest and Boosting models given the time constraint. It could also be due to the outliers or the noise in the training data, it could mean that the AR(5) model is able to capture the noise and the outliers better than the Random Forest and Boosting models.
...
hc_preds <- hc_fit %>% predict(gene_exp)
...
**Answer: ** The genes separated the samples into the two groups as shown in the dendrogram, however I suspect that it is due to the indication of num_clusters  = 2 in the hier_clust(). Similar results were produced by the average and complete linkage methods. However, the centroid linkage method produced a different result. The results do depend on the type of linkage used.
