...
The specific tract has high `crim` which would be considered an outlier. TODO: find the percentile of this tract.
...
The p-value for `zn` is less than 0.05, which suggests that there is a statistically significant association between the predictor and the response. And the coefficient suggests that it is a negative relationship between `zn` and `crim`, with -0.07393 coefficient, which suggests that per unit increase of the proportion of residential land zoned for lots over 25,000 sq.ft., per capita crime rate decreases by 0.07.
...
summary(multimodel)
```
As the output shows, `zn`, `dis`, `rad` and `medv` have p-values less than 0.05, which suggests that there is a statistically significant association between these predictors and `crim`. We can reject the null hypothesis H0 : $\beta{}_j = 0$ for these predictors.
